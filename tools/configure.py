...
def configure_test_env(contrail_fab_path='/opt/contrail/utils', test_dir='/contrail-test'):
    ...
    if api_auth_protocol == 'https':
        api_certfile = validate_and_copy_file(
            get_apiserver_certfile(), cfgm_host)
        api_keyfile = validate_and_copy_file(
            get_apiserver_keyfile(), cfgm_host)
    ...
    with settings(warn_only=True), hide('everything'):
        with lcd(contrail_fab_path):
            if local('git branch').succeeded:
                fab_revision = local(
                    'git log --format="%H" -n 1', capture=True)
            else:
    ...
    for host_string in env.roledefs['all']:
        ...
        if host_dict['data-ip'] == host_string.split('@')[1]:
            host_dict['data-ip'] = get_data_ip(host_string)[0]
        host_dict['control-ip'] = get_control_host_string(
            host_string).split('@')[1]
        ...
        if host_string in env.roledefs['openstack']:
            role_dict = {'type': 'openstack',
                         'params': {'cfgm': cfgm_host_name}}
            host_dict['roles'].append(role_dict)

        if host_string in env.roledefs['cfgm']:
            role_dict = {'type': 'cfgm', 'params': {
                'collector': host_name, 'cassandra': ' '.join(cassandra_host_names)}}
        ...
        if 'collector' in env.roledefs.keys() and host_string in env.roledefs['collector']:
            role_dict = {'type': 'collector', 'params': {
                'cassandra': ' '.join(cassandra_host_names)}}
            host_dict['roles'].append(role_dict)
    ...
    if env.has_key('vgw'):
        sanity_testbed_dict['vgw'].append(env.vgw)
    ...
    if esxi_hosts:
        for esxi in esxi_hosts:
            ...
            # Its not needed for vcenter_gateway mode, hence might not be there
            # in testbed.py
    ...
    stack_user = env.test.get('stack_user', os.getenv(
        'STACK_USER') or env.get('stack_user', ''))
    stack_password = env.test.get('stack_password',
                                  os.getenv('STACK_PASSWORD') or env.get('stack_password', ''))
    stack_tenant = env.test.get('stack_tenant', os.getenv(
        'STACK_TENANT') or env.get('stack_tenant', ''))
    stack_domain = env.test.get('stack_domain', os.getenv(
        'STACK_DOMAIN') or env.get('stack_domain', ''))
    ...
    discovery_ip = env.test.get(
        'discovery_ip', os.getenv('DISCOVERY_IP') or '')
    config_api_ip = env.test.get(
        'config_api_ip', os.getenv('CONFIG_API_IP') or '')
    ...
    control_port = env.test.get(
        'control_port', os.getenv('CONTROL_PORT') or '')
    dns_port = env.test.get('dns_port', os.getenv('DNS_PORT') or '')
    agent_port = env.test.get('agent_port', os.getenv('AGENT_PORT') or '')
    user_isolation = env.test.get('user_isolation', (os.getenv(
        'USER_ISOLATION')) or False if stack_user else True)
    ...
    use_devicemanager_for_md5 = getattr(
        testbed, 'use_devicemanager_for_md5', False)
    ...
    # Write vnc_api_lib.ini - this is required for vnc_api to connect to
    # keystone
