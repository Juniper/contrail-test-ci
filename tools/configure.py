...
def configure_test_env(contrail_fab_path='/opt/contrail/utils', test_dir='/contrail-test'):
    ...
    stack_tenant = env.test.get('stack_tenant', os.getenv('STACK_TENANT') or env.get('stack_user', ''))
    ...
    stack_domain = os.getenv('STACK_DOMAIN') or env.get('stack_domain', 'default-domain')
    ...
    if stack_user:
        user_isolation = env.test.get('user_isolation',
                                  bool(os.getenv('USER_ISOLATION') or False))
    else:
        user_isolation = env.test.get('user_isolation',
