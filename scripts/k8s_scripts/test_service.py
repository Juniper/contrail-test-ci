...
class TestService(BaseK8sTest):
    ...
    def test_service_access_from_different_ns(self):
        ''' Create a service  in one namespace with 2 pods
            ...
            Validate busybox pod can access the service 
            Both default and namsepace isolated case service
            should be accessible as service isolation not enabled
        '''
        expectation = True
        app = 'http_test'
        labels = {'app': app}
        ...
        assert self.verify_nginx_pod(pod2)
        assert pod3.verify_on_setup()
        # Now validate load-balancing on the service from different namespace
        #assert self.validate_nginx_lb([pod1, pod2], service.cluster_ip,
        #                              test_pod=pod3)
