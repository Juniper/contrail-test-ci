...
class Client():
    ...
    def set_isolation(self, namespace, enable=True):
        ...
        if enable:
            ...
            ns_obj.metadata.annotations['net.beta.kubernetes.io/network-policy'] = None
...
#    import pdb
#    pdb.set_trace()
    pol = c1.create_network_policy(name='test4',
            spec={
                  'pod_selector': {'match_labels': {'role': 'db'} },
                  'ingress': [ 
                    {
                    'from': [
#                        { 'namespace_selector':{'match_labels': {'project': 'myproject'} },
