...
class TrafficAnalyzer:
    ...
    def __init__(self, interface = None, compute_node_fixture=None,
        ...
        self.session = None
        self.pcap = None
        self.logger = logger or LOG.getLogger(__name__)
    ...
    def packet_capture_start(self, traffic_between_diff_networks = False,
        ...
        not self.node_ip and not self.password:
            self.logger.error("Either of interface, IP,"
    ...
    def verify_packets(self, packet_type, expected_count =None,
                       dot1p = None, dscp = None, mpls_exp = None,
                       pcap_path_with_file_name = None):
        '''
        ...
        if self.session == None:
            if not self.username and not self.node_ip and not self.password:
                ...
                self.session = ssh(self.node_ip, self.username,\
                                       self.password)
            if pcap_path_with_file_name:
                cmd = "ls -al %s" % pcap_path_with_file_name
            else:
                self.logger.debug("choosing default path to search .pcap")
                cmd = "ls -al '/tmp/*.pcap'"
            ...
            if out and len(out.split('\n'))-1 == 1:
                ...
                self.logger.error("None or Multiple pcap files exist")
                self.logger.error("Cannot parse pcap file")
                return False
        if expected_count:
        ...
        for ts,buff in pcap:
            ...
            self._check_underlay_interface_is_tagged():
                if dot1p:
                    string = ''
                    try:
                        vlan_header = bin(ether.tag)[2:]
                    except AttributeError, e:
                        self.logger.error(e)
                        return False
                    for i in range(0,16-len(vlan_header)):
                        string =string + '0'
                    vlan_header =string+ vlan_header
                    priority = int(vlan_header[:3],2)
                    if priority == dot1p:
