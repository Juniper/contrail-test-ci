...
#                    uid = (id,'IfmapServer')
    ...
    def modify_discovery_conf_file_params(self, operation, **args):
        ...
        6. set_auto_load_balance Vars: publisher_type = None, load_balance_flag = False
        7. set_policy Vars: publisher_type = None , policy = "load-balance"
        8. del_policy Vars: publisher_type = None
        '''
        final_status = True
        ...
        elif operation == "set_auto_load_balance":
            # Setting and resetting load balance on specified publisher
            # publisher-type : type of service (eg: XMPP-SERVER, DNS-SERVER, OPSERVER)
            publisher_type = args.get('publisher_type',None)
            load_balance_flag = args.get('load_balance_flag',True)
            if not publisher_type:
                self.info.error("Publisher type not mentioned while setting load balance")
                final_status = False
            else:
                self.modify_conf_file('set', 'contrail-discovery', publisher_type, \
                              'load-balance', load_balance_flag)
    ...
    def white_list_publishers_conf_file(self, *publisher_ips):
        ...
        self.logger.debug("List of white list publishers is %s" % publisher_list)
        self.modify_conf_file('set', 'contrail-discovery', 'DEFAULTS', \
                    'white_list_publish',publisher_list)
        conf_file = '/etc/contrail/contrail-discovery.conf '
        cmd = 'cat ' + conf_file
        for ip in self.inputs.cfgm_ips:
            out_put = self.inputs.run_cmd_on_server(ip, cmd, \
    ...
    def white_list_subscribers_conf_file(self, *subscriber_ips):
        ...
        self.logger.debug("List of white list subscribers is %s" % subscriber_list)
        self.modify_conf_file('set', 'contrail-discovery', 'DEFAULTS', \
                    'white_list_subscribe',subscriber_list)
        conf_file = '/etc/contrail/contrail-discovery.conf '
        cmd = 'cat ' + conf_file
        for ip in self.inputs.cfgm_ips:
            out_put = self.inputs.run_cmd_on_server(ip, cmd, \
...
# match two rules (type DiscoveryServiceAssignmentType)
    ...
    def verify_rule_creation_delete_read(self, ds_ip=None):
        ...
        for i in range(0,len(self.inputs.cfgm_control_ips)):
            ...
            self.discovery_rule_config("add_rule",\
        ...
        for i in range(0,len(self.inputs.cfgm_control_ips)): 
            ...
            cfgm_control_ip[3] = '0'
            cfgm_control_ip = ".".join(cfgm_control_ip) + "/24"
            self.discovery_rule_config('del_rule',\
                'default-discovery-service-assignment', cfgm_control_ip,\
                'xmpp-server', cfgm_control_ip,'contrail-vrouter-agent:0')
            self.discovery_rule_config('del_rule',\
                'default-discovery-service-assignment', cfgm_control_ip,\
    ...
    def verify_rule_xmpp_server_vrouter_agent(self, ds_ip=None):
        ...
        assert self.modify_discovery_conf_file_params("change_min_max_ttl",\
                                                ttl_min=60, ttl_max=60)
        ...
        for i in range(0,len(self.inputs.bgp_control_ips)):
            ...
            self.discovery_rule_config( "add_rule",\
                'default-discovery-service-assignment', bgp_control_ip,'xmpp-server',\
        ...
        for i in range(0,len(self.inputs.bgp_control_ips)): 
            ...
            bgp_control_ip = ".".join(bgp_control_ip) + "/24"
            result1 = self.discovery_rule_config( "find_rule",\
                'default-discovery-service-assignment',bgp_control_ip,\
        ...
        assert self.modify_discovery_conf_file_params(operation= 'change_min_max_ttl')
        return result
    ...
    def verify_rule_dns_server_vrouter_agent(self, ds_ip=None):
        ...
        for i in range(0,len(self.inputs.bgp_control_ips)):
            ...
            bgp_control_ip = ".".join(bgp_control_ip) + "/24"
            self.discovery_rule_config( "add_rule",\
                'default-discovery-service-assignment', bgp_control_ip, 'dns-server',\
                 bgp_control_ip, 'contrail-vrouter-agent:0')
        self.read_rule('default-discovery-service-assignment')
        for i in range(0,len(self.inputs.bgp_control_ips)): 
            bgp_control_ip = self.inputs.bgp_control_ips[i].split('.')
    ...
    def verify_subscribe_request_with_diff_instances_rules(self, ds_ip=None):
        ...
        try:
            ...
            self.read_rule('default-discovery-service-assignment')
            result1 = self.discovery_rule_config( "find_rule",\
                    'default-discovery-service-assignment',self.inputs.cfgm_control_ips[0],\
    ...
    def verify_rule_when_service_admin_down(self, ds_ip=None):
        ...
        try:
            ...
            self.read_rule('default-discovery-service-assignment')
            result1 = self.discovery_rule_config( "find_rule",\
                    'default-discovery-service-assignment',self.inputs.bgp_control_ips[0],\
            ...
            self.update_service(ds_ip,service="dns-server",\
    ...
    def verify_multiple_rule_same_subscriber(self, ds_ip=None):
        ...
        try:
            ...
            self.discovery_rule_config( "add_rule",\
                'default-discovery-service-assignment', \
                self.inputs.bgp_control_ips[1],'xmpp-server',\
                self.inputs.compute_control_ips[0], 'contrail-vrouter-agent:0')
            self.read_rule('default-discovery-service-assignment')
            result1 = self.discovery_rule_config( "find_rule",\
                    'default-discovery-service-assignment', self.inputs.bgp_control_ips[1],\
                    'xmpp-server', self.inputs.compute_control_ips[0], 'contrail-vrouter-agent:0')
            if result1 == False:
                self.logger.error("# While searching for the configured rule, it was not found. Configuration failed #")
                result = False
    ...
    def verify_rule_with_vrouter_agent_do_not_impact_other_subscriptions(self, ds_ip=None):
        ...
        try:
            ...
            xmpp_vrouter_subscription_list = self.get_all_xmpp_servers(ds_ip)
            self.logger.info("# Create a rule for dns-server running on control node and subscriber as contrail-vrouter-agent   #")
            ...
            for i in range(1,90):
                new_xmpp_vrouter_subscription_list=self.get_all_xmpp_servers(ds_ip)
                sleep(1)
    ...
    def verify_discovery_server_restart_rule_present(self, ds_ip=None):
        ...
        try:
            ...
            for i in range(0,len(self.inputs.bgp_control_ips)):
                verification = self.verify_client_subscription_to_expected_publisher\
                                (ds_ip, 'contrail-control',\
                                self.inputs.bgp_control_ips[i], 'IfmapServer')
                if verification == False:
                    self.logger.error("Rule not behaving as expected")
                    result = False
    ...
    def verify_auto_load_balance_Ifmap(self, ds_ip=None):
        ...
        assert self.modify_discovery_conf_file_params( 'set_auto_load_balance',\
                    publisher_type = 'IFMAPSERVER',load_balance_flag = 'TRUE')
        try:
            ...
            sleep(100)
            count=self.get_service_in_use(ds_ip,(self.inputs.cfgm_control_ips[0],\
                                                 'IfmapServer'))
            if count == 0:
                pass
            else:
        ...
        assert self.modify_discovery_conf_file_params(operation='set_auto_load_balance',\
                        publisher_type = 'IFMAPSERVER', load_balance_flag='False')
        try:
    ...
    def verify_auto_load_balance_xmpp(self, ds_ip=None):
        ...
        self.logger.info("# Setting auto load balance to true in contrail-discovery.conf file #")  
        assert self.modify_discovery_conf_file_params(operation='set_auto_load_balance',\
                    publisher_type='XMPP-SERVER',load_balance_flag='True')
        try:
            ...
            sleep(20)
            count=self.get_service_in_use(ds_ip,(self.inputs.bgp_control_ips[0],\
                                                 'xmpp-server'))
            if count == 0:
        ...
        self.logger.info("# Setting auto load balance to False in contrail-discovery.conf file #")  
        assert self.modify_discovery_conf_file_params( 'set_auto_load_balance',\
                publisher_type='XMPP-SERVER',load_balance_flag = 'False')
        try:
    ...
    def verify_auto_load_balance_collector(self, ds_ip=None):
        ...
        self.logger.info("# Setting auto load balance to true in contrail-discovery.conf file #")  
        assert self.modify_discovery_conf_file_params(operation='set_auto_load_balance',\
                publisher_type='COLLECTOR',load_balance_flag='True')
        try:
            ...
            self.logger.debug("# Waiting for 100 seconds to wait for server to lose all subscriptions #")
            sleep(100)
            count=self.get_service_in_use(ds_ip,\
        ...
        self.logger.info("# Setting auto load balance to False in contrail-discovery.conf file #")  
        assert self.modify_discovery_conf_file_params(operation='set_auto_load_balance',\
                publisher_type='COLLECTOR',load_balance_flag='False')
        try:
    ...
    def verify_service_in_use_list(self, ds_ip=None):
        ...
        try:
            ...
            in_use_list = []
            for i in range(0,len(self.inputs.cfgm_control_ips)): 
                in_use_list_elem = self.get_service_in_use(ds_ip,\
    ...
    def verify_white_list_security(self, ds_ip=None):
        ...
        try:
            ...
            response = self.publish_service_to_discovery(ds_ip,\
            ...
            if self.get_all_services_by_service_name(ds_ip,\
                                             service="Test_Pub_2") == []:
            ...
            assert self.modify_discovery_conf_file_params( 'delete_white_list',\
    ...
    def verify_keystone_auth_security(self, ds_ip=None):
        ...
        try:
            ...
            assert self.modify_discovery_conf_file_params('add_keystone_auth',\
            ...
            response = self.publish_requests_with_keystone(ds_ip,\
            ...
            assert self.modify_discovery_conf_file_params(operation='add_keystone_auth',\
        ...
        assert self.modify_discovery_conf_file_params(operation='delete_keystone_auth'\
                                               ,auth="keystone")       
        return result
    ...
    def verify_policy_fixed(self, ds_ip=None):
        ...
        try:
            ...
            self.subscribe_service_from_discovery(ds_ip,service="TEST_PUB",\
            ...
            assert self.modify_discovery_conf_file_params( 'del_policy',\
    ...
    def verify_rule_do_not_affect_other_dns_subscriptions(self, ds_ip=None):
        ...
        for i in range(0,len(self.inputs.compute_control_ips)):  
            client_subscribed_service_id=self.get_subscribed_service_id(ds_ip,\
