...
#        if 1 == len (avn):
    ...
    def get_vrouter_nh(self, index):
        ...
        if not xml_obj or len(xml_obj) != 1:
            self.log.debug('Unable to fetch nh details in vrouter for '
                ' index %s, Got :%s' % (index, xml_obj))
            return None
        return self._make_dict_from_tag_text(xml_obj[0])
    # end get_vrouter_nh

    def _make_dict_from_tag_text(self, xml_obj):
        '''
        '''
        p = {}
        for e in xml_obj:
    ...
    def get_vrouter_route_table(self, vrf_id, get_nh_details=False, **kwargs):
        ...
        for obj in xml_obj:
            ...
            for route in routes_elem:
                p = self._make_dict_from_tag_text(route)
                if filter_set.issubset(p.items()):
                    if get_nh_details:
    ...
    def get_nh_for_route_in_vrouter(self, prefix, route_table=[], **kwargs):
        ...
        nh = None
        (prefix_1, prefix_2) = prefix.split('/')
        if not route_table:
            vrf_id = kwargs.get('vrf_id')
            route_table = self.get_vrouter_route_table(vrf_id, prefix=prefix_1,
                                               prefix_len=prefix_2,
                                               get_nh_details=True)

        for route in route_table:
            if route['prefix'] == prefix_1 and \
                route['prefix_len'] == prefix_2:
