...
def get_vcenter_connection(inputs):
    SI = None
    try:
        SI = connect.SmartConnect(host=inputs.vcenter_server, port=int(inputs.vcenter_port), user=inputs.vcenter_username, pwd=inputs.vcenter_password)
    ...
    if not SI:
            raise Exception("Unable to connect to vcenter: %s:%d %s/%s" % (inputs.vcenter_server,
                            inputs.vcenter_port, inputs.vcenter_username, inputs.vcenter_password))
    return SI
