...
class GWLessFWDTestBase(BaseVrouterTest, ConfigSvcChain):
    ...
    def provison_bgp_peer(self, bgp=None):
        ...
        for i in range(0,bgp_count):
            ...
            if bgp_id in bgp:
                ...
                params.address_families = vnc_api_test.AddressFamilies(
                    address_families)
                ...
                try:
                    ...
                    self.logger.error("%s Error is configuring BGP router " %(
                        router_name))
                    return False
    ...
    def setup_gw_less_fwd(self, vn=None, vmi=None, vm=None, bgp=None,
                          verify=True):
                ...
                if vn_fixtures[vn_name].is_ip_fabric_provider_nw_present():
                    ...
                    # Source VM is remote VM
                        ...
                        result = verify_tcpdump_count(self, session, pcap,
                                                      exp_count=2,
                                                      grep_string=src_vm_fixture.vm_ip)
                        ...
                        if result:
                            ...
                            assert result, "Packets are going NOT through "\
                                ...
                                "%s and VM: %s" %(compute_ip, src_vm_fixture.vm_ip)
                    ...
                    if expectation:
                        ...
                        if src_vn_name != dst_vn_name:
                            if result:
                                ...
                                "should not be successful" %(src_vm_ip, dst_vm_ip)
                            else:
                                self.logger.info('Ping from VM: %s to VM: %s '\
                                                 'is NOT successful as expected'
                                                 %(src_vm_ip, dst_vm_ip))
    ...
    def config_fip(self, ret_dict, vn_id, vmi_id):
        '''
        ...
        fip_fixture = self.useFixture(
            FloatingIPFixture(
                ...
                connections=self.connections, pool_name='some-pool1', vn_id=vn_id))
        assert fip_fixture.verify_on_setup()

        fvn_obj = self.vnc_lib.virtual_network_read(id=vn_id)
        ...
        vm_intf = self.vnc_lib.virtual_machine_interface_read(id=vmi_id)
        ...
        fip_obj = self.vnc_lib.floating_ip_read(fq_name=fip_obj.fq_name)
