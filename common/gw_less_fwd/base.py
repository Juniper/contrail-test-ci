...
class GWLessFWDTestBase(BaseVrouterTest, ConfigSvcChain):
            ...
            for cn in src_vm_fixture.get_control_nodes():
                ...
                for vm_ip in src_vm_fixture.get_vm_ip_dict()[vn_fq_name]:
                    ...
                    if vn_fixtures[vn_name].is_ip_fabric_provider_nw_present():
                        if cn_routes:
                            self.logger.info("Route: %s is found in default routing instance as expected " % vm_ip)
                        else:
                            result = False
                            assert result, "No route: %s found in Control-node %s, not expected" % (vm_ip, cn)
                    else:
                        if not cn_routes:
                            self.logger.info("Route: %s is NOT found in default routing instance as expected " % vm_ip)
                        else:
                            result = False
                            assert result, "Route: %s found in Control-node %s, not expected" % (vm_ip, cn)
    ...
    def verify_routes_ip_fabric_vn_in_agent(self, ret_dict = None):
        ...
        for src_vm_fixture in vm_fixtures.values():
            ...
            if vn_fixtures[vn_name].is_ip_fabric_provider_nw_present():
                if route:
                    self.logger.info("Route: %s is found in default routing instance as expected " % src_vm_fixture.vm_ip)
                else:
                    result = False
                    assert result, "Route: %s is NOT present in fabric routing instance, not expected" % src_vm_fixture.vm_ip
            else:
                if not route:
                    self.logger.info("Route: %s is NOT found in default routing instance as expected " % src_vm_fixture.vm_ip)
                else:
                    result = False
                    assert result, "Route: %s is present in fabric routing instance, not expected" % src_vm_fixture.vm_ip
        return True


    def verify_ping_from_vhosts_to_vms(self, ret_dict = None, expectation=True, mode = "underlay"):
        '''
        ...
        for compute_ip in compute_node_ips:
            for src_vm_fixture in vm_fixtures.values():
                ...
                # In overlay mode i.e IP fabric forwarding is disabled, ping
                # from vhost to VM should fail.
                if mode == "overlay":
                    ...
                    if result:
                        ...
                        assert not result, 'Ping from compute node: %s to VM: %s is successful, not expected' %(compute_ip, src_vm_fixture.vm_ip)

                # Underlay mode. Pinging local VM from compute node.
                # ping should be successful
                elif src_vm_fixture.get_compute_host() == compute_ip:
                ...
                # pinging remote compute VM from compute in underlay mode. Verify whether
                # traffic goes through underlay or not
                    ...
                    if expectation:
                        if result:
                            self.logger.info('Ping from compute node: %s to VM: %s is successful' %(compute_ip, src_vm_fixture.vm_ip))
                        else:
                            assert result, 'Ping from compute node: %s to VM: %s is NOT successful' %(compute_ip, src_vm_fixture.vm_ip)
                    else:
                        if result:
                            assert not result, 'Ping from compute node: %s to VM: %s should NOT be successful' %(compute_ip, src_vm_fixture.vm_ip)
                        else:
                            self.logger.info('Ping from compute node: %s to VM: %s is NOT successful as expected' %(compute_ip, src_vm_fixture.vm_ip))
    ...
    def verify_ping_from_vms_to_vhosts(self, ret_dict = None, expectation=True, mode="underlay"):
        '''
            ...
            for compute_ip in compute_node_ips:
                ...
                if mode == "overlay":
                    ...
                    if result:
                        ...
                        assert not result, 'Ping from VM: %s to compute node: %s is successful, which is not expected' %(src_vm_fixture.vm_ip, compute_ip)

                # Underlay mode. Pinging local compute node from VM.
                # ping should be successful
                elif src_vm_fixture.get_compute_host() == compute_ip:
                ...
                # traffic goes through underlay or not
                    ...
                    if expectation:
                        ...
                        if result:
                            assert not result, 'Ping from VM: %s to remote compute node: %s should NOT be successful, not expected' %(src_vm_fixture.vm_ip, compute_ip)
                        else:
                            self.logger.info('Ping from VM: %s to remote compute node: %s is NOT successful as expected' %(src_vm_fixture.vm_ip, compute_ip))
    ...
    def verify_ping_across_vms(self, ret_dict = None, expectation=True, mode="underlay"):
        '''
        ...
        for src_vm_fixture in vm_fixtures.values():
            for dst_vm_fixture in vm_fixtures.values():
                ...
                # mode is overlay or both src VM and dst VM are in same compute.
                if mode == "overlay" or (src_vm_fixture.get_compute_host() == dst_vm_fixture.get_compute_host()):
                ...
                # mode is underlay and dest VM is in remote compute. Traffic
                # should go through underlay.
                else:
    ...
    def verify_gw_less_fwd(self, ret_dict = None, mode = "underlay"):
        '''
        ...
        # Pinging all the VMIs. Traffic should go through underlay
        if policy:
            self.verify_ping_across_vms(ret_dict=ret_dict, expectation=True, mode=mode)
        else:
            self.verify_ping_across_vms(ret_dict=ret_dict, expectation=False, mode=mode)

        if mode == "underlay":
            # Pinging all VMIs from vhost. Traffic should go through underlay
            self.verify_ping_from_vhosts_to_vms(ret_dict=ret_dict, expectation=True, mode=mode)
        else:
            # Pinging all VMIs from vhost. Traffic should fail
            self.verify_ping_from_vhosts_to_vms(ret_dict=ret_dict, expectation=False, mode=mode)

        if mode == "underlay":
            # Pinging all vhosts from VMIs. Traffic should go through underlay
            self.verify_ping_from_vms_to_vhosts(ret_dict=ret_dict, expectation=True, mode=mode)
        else:
            # Pinging all vhosts from VMIs. Traffic should fail
            self.verify_ping_from_vms_to_vhosts(ret_dict=ret_dict, expectation=False, mode=mode)
