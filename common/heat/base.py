...
class BaseHeatTest(test_v1.BaseTestCase_v1):
    ...
    def setUpClass(cls):
        ...
        if cls.inputs.domain_isolation:
            cls.domain_name = cls.connections.domain_name or cls.inputs.domain_name
        cls.project_fq_name = [cls.domain_name or cls.inputs.domain_name ,cls.inputs.project_name]
    ...
    def config_svc_template(self, stack_name=None, scaling=False, mode='in-network-nat'):
        ...
        env['parameters']['name'] = get_random_name(stack_name)
        env['parameters']['domain'] = self.domain_name or 'deafult-domain'
    ...
    def verify_st(self, env, scaling, ver):
        ...
        st_fix = self.useFixture(SvcTemplateFixture(
            connections=self.connections, inputs=self.inputs, domain_name=self.domain_name or 'default-domain',
