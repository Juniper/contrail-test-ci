...
class BaseK8sTest(test.BaseTestCase, _GenericTestBaseMethods, vnc_api_test.VncLibFixture):
    ...
    def create_router(self, name):
        ...
        obj = vnc_api_test.LogicalRouter(name=name, parent_obj=self.connections.vnc_lib_fixture.get_project_obj(),
                                         display_name=name)

        self.vnc_lib.logical_router_create(obj)

        self.addCleanup(self.router_delete, obj)
        return obj 

    def router_delete(self, router_obj):

        self.vnc_lib.logical_router_delete(id=router_obj.uuid)
    ...
    def connect_gateway_with_router(self, router_obj):
 
        # Add public network to router as external_gateway
        router_obj.add_virtual_network(self.public_vn.public_vn_fixture.obj)
