...
class QosTestBase(BaseNeutronTest):
    ...
    def validate_queue_performance(self,
        ...
        cmd = "for i in `ls /sys/class/net/%s/queues/tx*/xps_cpus`;do echo 0 > $i;done"\
                     % interface
        self.inputs.run_cmd_on_server(src_vn1_vm1_fixture.vm_node_ip, cmd )
        ...
        if queue_types == "rr":
            ...
            error_margin = expected_ratio_q1_q2 * 0.1 # Setting 10% error margin
            ratio_diff = abs(traffic_ratio - expected_ratio_q1_q2)
            if ratio_diff <= error_margin:
    ...
    @classmethod
    def populate_qos_queue_params(cls, node_ip):
        '''
        This module will populate multi queueing interface, it's speed and 
    ...
    def populate_configured_queue_mapping(self, node_ip):
        '''
        This module will populate list of HW queues.
    ...
    def populate_all_configured_logical_ids(self, node_ip):
        '''
        This module will populate list logical queues.
    ...
    def pick_nodes(cls):
        ...
        for elem in cls.inputs.qos_queue:
            ...
            cls.queue_params = cls.populate_qos_queue_params(cls.qos_node_ip)
        ...
        for elem in cls.inputs.compute_ips:
            if elem != cls.qos_node_ip:
                ...
                second_node_ip = cls.inputs.compute_names[1]
    ...
    def match_traffic(self, init_pkts, final_pkts, min_expected_pkts):
        ...
        if total_packets == 0:
            ...
            return False
        error_margin = min_expected_pkts * 0.01
        packet_diff = abs(total_packets - min_expected_pkts)
        self.logger.debug("Diff of actual and expectd packets is %d" % 
                          packet_diff)
        if packet_diff <= error_margin or total_packets > min_expected_pkts:
    ...
    def configure_queue_features(cls):
        ...
        '''
        interface = cls.populate_qos_queue_params(cls.qos_node_ip)[0]
...
class TestQosQueueProperties(QosTestExtendedBase):
    ...
    def setUpClass(cls):
        ...
        cls.vn2_vm2_fixture = cls.create_only_vm(cls.vn2_fixture,
                                  node_name=cls.first_node_name)
        cls.check_vms_booted([cls.vn2_vm2_fixture])
    # end setUpClass
