...
class Edit:
    ...
    def add_subinterface_ports(self, option, port_name, params_list, tc='positive'):
        result = True
        try:
            self.sub_interface = self.edit_remove_option(option, 'subinterface',
                                                        display_name=port_name)
            if self.sub_interface:
                self.click_element('s2id_virtualNetworkName_dropdown')
                if not self.select_from_dropdown(params_list[0], grep=False):
                    result = result and False
                self.send_keys(params_list[1], 'display_name', 'name')
                self.click_element('advanced_options')
                self.send_keys(params_list[2], 'sub_interface_vlan_tag', 'name')
