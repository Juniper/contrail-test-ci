...
class BaseVrouterTest(BaseNeutronTest):
    ...
    def verify_traffic_for_ecmp_si(self, sender_vm_fix, si_vm_list,
        ...
        #Verify tcpdump count, any one SI should receive the packet
        for vm in si_vm_list:
            if not self.inputs.pcap_on_vm:
            ...
            if ret:
                self.logger.info("Tcpdump verification on SI %s passed" %
                                    vm.vm_ip)
                result = ret
                break
    ...
    def verify_ecmp_routes(self, vm_fix_list, prefix):
        ...
        if result:
            if not tap_itf_list:
                ...
                self.logger.debug("Tap interface %s not found in any agent" % (
                    tap_itf_list))
                return False
        else:
            self.logger.debug("ECMP routes not found in any agent")
            return False
