...
class BaseVrouterTest(BaseNeutronTest):
    ...
    def create_vns(self, count=1, **kwargs):
        vn_fixtures = []
        for i in xrange(count):
            vn_subnets = get_random_cidrs(self.inputs.get_af())
            vn_fixtures.append(self.create_vn(vn_subnets=vn_subnets, **kwargs))
    ...
    def create_vms(self, vn_fixture, count=1, image_name='ubuntu', **kwargs):
        vm_fixtures = []
        for i in xrange(count):
    ...
    def start_ping(self, src_vm, dst_vm=None, dst_ip=None, wait=False,
        ...
        ping_h = Ping(src_vm, dst_ip, **kwargs)
    ...
    def verify_flow_action(self, compute_fix, action, src_ip=None, dst_ip=None,
        ...
        '''
        (forward_flow, reverse_flow) = compute_fix.get_flow_entry(
            source_ip=src_ip, dest_ip=dst_ip, source_port=sport,
            dest_port=dport, proto=proto, vrf_id=src_vrf)
