...
class BaseVrouterTest(BaseNeutronTest):
    ...
    def verify_traffic_load_balance(self, sender_vm_fix,
        ...
        for i in xrange(try_count):
            result = True
            baseport = str(random.randint(12000, 65000))

            #Get dropstats on each computes before starting traffic
            for fixture in compute_fixtures:
                dropstats[fixture.ip] = fixture.get_vrouter_dropstats()
            ...
            #If loss is seen by hping3 and tcpdump verification also fails then assert
            if stats['loss'] != '0' and not result:
                for fixture in compute_fixtures:
                    self.logger.debug("dropstats on node: %s before test: %s"
                                         % (fixture.ip,
                                         dropstats[fixture.ip]))
                    dropstats[fixture.ip] = fixture.get_vrouter_dropstats()
                    self.logger.debug("dropstats on node: %s after test: %s"
                                         % (fixture.ip,
                                         dropstats[fixture.ip]))
