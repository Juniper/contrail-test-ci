...
class BaseDropStats(GenericTestBase) :
    ...
    def get_drop_stats_dict_from_cmd(self, compute, module='vrouter', intf_id=None):
        self.agent_inspect_h[compute].get_agent_vm_interface()
        cmd = 'dropstats'
        if module == 'vif':
    ...
    def get_drop_stats_dict_from_introspect(self, compute, ip=None, module='vif'):
        if module == 'vrouter':
            drop_stats_list = self.agent_inspect_h[compute].get_agent_vrouter_drop_stats()[0]
        else:
            drop_stats_list = self.agent_inspect_h[compute].get_agent_vm_interface_drop_stats(ip)[0]
    ...
    def verify_flow_action_drop_stats(self, drop_type='ds_flow_action_drop'):
        ...
        vif_dict_before = self.get_drop_stats_dict_from_introspect(compute0, ip_addr)
        self.print_drop_stats_dict(vif_dict_before)

        vrouter_dict_before = self.get_drop_stats_dict_from_introspect(compute0, module='vrouter')
        ...
        count = 0
        while True:
            vif_dict_after = self.get_drop_stats_dict_from_introspect(compute0, ip_addr)
            vrouter_dict_after = self.get_drop_stats_dict_from_introspect(compute0, module='vrouter')
