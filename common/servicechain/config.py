...
class ConfigSvcChain(fixtures.TestWithFixtures):
    ...
    def config_st_si(self, st_name, si_name_prefix, si_count,
                     ...
                     svc_img_name=None, st_version=1, vm_launch_mode=None):
        ...
        st_fixture = self.useFixture(SvcTemplateFixture(
            ...
            flavor=flavor, ordered_interfaces=ordered_interfaces,
            version=st_version))
        ...
        for i in range(0, si_count):
            ...
            if st_version == 2:
                ...
                for i in range(max_inst):
                    ...
                    # Incase, vm_launch_mode is distribute, distribute the SVMs
                    # if it is non-distribute, launch all the SVMs on one node
                    if vm_launch_mode == 'distribute':
                        node_name = compute_nodes[i%compute_nodes_len]
                    elif vm_launch_mode == 'non-distribute':
                    ...
                    if svc_mode == 'transparent':
                        svm_fixture = self.config_and_verify_vm(
                            ...
                            vns=[self.trans_mgmt_vn_fixture,
                                 ...
                                 self.trans_right_vn_fixture],
                            count=1, flavor='m1.large', node_name=node_name)
                    else:
                        svm_fixture = self.config_and_verify_vm(
                            svm_name, image_name=svc_img_name,
    ...
    def config_and_verify_vm(self, vm_name, vn_fix=None, image_name='ubuntu-traffic', vns=[], count=1, flavor='contrail_flavor_small', node_name=None):
        if vns:
            ...
            vm_fixture = self.config_vm(
                ...
                flavor=flavor, node_name=node_name)
        else:
            vm_fixture = self.config_vm(
    ...
    def config_vm(self, vm_name, vn_fix=None, node_name=None, image_name='ubuntu-traffic', flavor='contrail_flavor_small', vns=[], count=1):
        if vn_fix:
            vm_fixture = self.useFixture(VMFixture(
                ...
                image_name=image_name, flavor=flavor, count=count))
        elif vns:
            vm_fixture = self.useFixture(VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vm_name=vm_name, node_name=node_name, image_name=image_name,
                flavor=flavor, vn_objs=vns, count=count))
