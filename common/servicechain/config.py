...
#    def config_st_si(self, st_name, si_name_prefix,
#                     svc_scaling=False, max_inst=1,
#                     mgmt_vn_fixture=None, left_vn_fixture=None,
#                     right_vn_fixture=None, svc_type='firewall',
#                     svc_mode='transparent', flavor='contrail_flavor_2cpu',
#                     static_route=None,
#                     svc_img_name=None, st_version=1):
#
#        static_route = static_route or { 'management':None,
#                                         'left' : None,
#                                         'right': None}
#        left_scaling = False
#        right_scaling = False
...
#
#        mgmt_vn_fq_name = mgmt_vn_fixture.vn_fq_name if mgmt_vn_fixture else None
#        left_vn_fq_name = left_vn_fixture.vn_fq_name if left_vn_fixture else None
#        right_vn_fq_name = right_vn_fixture.vn_fq_name if right_vn_fixture else None
#
#        if_details = {
#            'management' : { 'shared_ip_enable' : False,
#                             'static_route_enable': False,
#                             'vn_name' : mgmt_vn_fq_name},
#            'left' : { 'shared_ip_enable': left_scaling,
#                       'static_route_enable' : bool(static_route['left']),
#                       'vn_name' : left_vn_fq_name},
#            'right' : { 'shared_ip_enable': right_scaling,
#                       'static_route_enable' : bool(static_route['right']),
#                       'vn_name' : right_vn_name}}
##        if_list = [mgmt_props,
...
#                          'image: %s' "\n"
#                          'flavor: %s' "\n"
#                          'intf_list: %s' % (svc_type, svc_mode, svc_img_name, flavor, if_details))
#        # create service template
#        st_fixture = self.useFixture(SvcTemplateFixture(
#            connections=self.connections,
#            st_name=st_name, svc_img_name=svc_img_name, svc_type=svc_type,
#            if_details=if_details, svc_mode=svc_mode, svc_scaling=svc_scaling, flavor=flavor, ordered_interfaces=ordered_interfaces, version=st_version))
#        assert st_fixture.verify_on_setup()
#
#
...
#                mgmt_vn_name = get_random_name('trans_mgmt_vn')
#                _mgmt_vn_subnets = [
#                    get_random_cidr(af=self.inputs.get_af())]
#                left_vn_name = get_random_name('trans_left_vn')
#                left_vn_subnets = [
#                    get_random_cidr(af=self.inputs.get_af())]
#                right_vn_name = get_random_name('trans_right_vn')
#                right_vn_subnets = [
#                    get_random_cidr(af=self.inputs.get_af())]
#                mgmt_vn_fixture = self.config_vn(
#                    mgmt_vn_name, mgmt_vn_subnets)
#                left_vn_fixture = self.config_vn(
#                    left_vn_name, left_vn_subnets)
#                right_vn_fixture = self.config_vn(
#                    right_vn_name, right_vn_subnets)
...
#                if mgmt_vn_fixture:
#                    svm_pt_props['management'] = svm_fixture.vmi_ids[mgmt_vn_fixture.vn_fq_name]
#                if left_vn_fixture:
#                    svm_pt_props['left'] = svm_fixture.vmi_ids[left_vn_fixture.vn_fq_name]
#                if right_vn_fixture:
#                    svm_pt_props['right'] = svm_fixture.vmi_ids[right_vn_fixture.vn_fq_name]
#                si_fixture.add_port_tuple(svm_pt_props)
#        si_fixture.verify_on_setup()
#
#        return (st_fixture, si_fixture)
#
