    ...
    def verify_intf_mirroring(self, compute_nodes, vn_index_list, sub_intf=False):
        ...
        if sub_intf:
            try:
                ...
                self.src_vm_ip = self.src_port.neutron_handle.get_port(self.src_port.uuid)['fixed_ips'][0]['ip_address']
                self.dst_vm_ip = self.dst_port.neutron_handle.get_port(self.dst_port.uuid)['fixed_ips'][0]['ip_address']
                self.analyzer_vm_ip = self.analyzer_port.neutron_handle.get_port(self.analyzer_port.uuid)['fixed_ips'][0]['ip_address']
        else:
