...
class VerifyIntfMirror(VerifySvcMirror):
    ...
    def verify_intf_mirroring(self, compute_nodes, vn_index_list, sub_intf=False):
        ...
        if not sub_intf:
            ...
            self.src_vm_fixture = self.create_vm(vn_objs=[self.src_parent_port_vn_fixture.obj], vm_name=self.src_vm_name,
                                     image_name=image_name, node_name=src_compute, port_ids=src_port_ids)

            self.dst_vm_fixture = self.create_vm(vn_objs=[self.dst_parent_port_vn_fixture.obj], vm_name=self.dst_vm_name,
                                     image_name=image_name, node_name=dst_compute, port_ids=dst_port_ids)

            self.analyzer_vm_fixture = self.create_vm(vn_objs=[self.analyzer_parent_port_vn_fixture.obj], vm_name=self.analyzer_vm_name,
                                     image_name=image_name, node_name=analyzer_compute, port_ids=analyzer_port_ids)
