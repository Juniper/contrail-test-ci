...
from common.openstack_libs import v3,session
...
class KeystoneCommands():
    ...
    def __init__(self, username=None, password=None, tenant=None,
        ...
        if token:
            ...
            if (auth_url is not None) and (inputs is not None) and (inputs.domain_isolation is True):
    ...
    def get_session(self):
        '''if 'v3' in self.keystone.auth_url:
            auth = v3.Password(auth_url=self.keystone.auth_url,
                   username=self.keystone.username,
                    password=self.keystone.password,
                    project_name=self.keystone.project_name,
                    user_domain_id=self.keystone.domain_id,
                    project_domain_id=self.keystone.domain_id)
    ...
    def get_domain_id(self, domain_name):        
       try:        
           obj =  self.keystone.find_domain(name=name)        
