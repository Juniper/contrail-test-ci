...
#            assert False
    ...
    def stop_webserver(self, listen_port=".*"):
        ...
        try:
            vm_host_string = '%s@%s'%(self.vm_username, self.local_ip)
            ##cmd = 'echo %s >& index.html'%(content or self.vm_name)
            cmd = "ps -ef | grep HTTP | grep Simple | grep %s | awk '{print $2}'" %listen_port
            ...
            psid=[out for out in output if "unable" not in out]
            self.logger.info("PS id is %s" %psid)
            for ps in psid:
                cmd = 'kill -9 %s' %ps
                output = remote_cmd(
                    vm_host_string, cmd, gateway_password=host['password'],
