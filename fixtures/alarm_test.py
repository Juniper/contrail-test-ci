...
class AlarmFixture(fixtures.Fixture):
    ...
    def verify_alarm_in_api_server(self):
        ...
        if not self.cs_alarm:
            self.logger.info('Alarm config of %s not present in api server' %self.alarm_name)
            return False
        self.logger.info('Alarm config of %s present in the api-server' %self.alarm_name)
        return True
    ...
    def verify_alarm_setup(self):
        ...
        if not result:
            self.logger.error('Verification of %s config in api-server failed'%self.alarm_name)
            return result
