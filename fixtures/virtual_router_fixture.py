    ...
    def update_virtual_router_type(self,vrouter_type=None):
        if not vrouter_type:
            try:
                ...
                self.logger.error("Virtual router %s update failed for type %s"%(self.name,self.virtual_router_type)) 
        else:
            try:
                self.vnc_h.update_virtual_router_type(self.name,vrouter_type)
            except Exception as e:
                self.logger.error("Virtual router %s update failed for type %s"%(self.name,vrouter_type)) 
