...
class VNFixture(fixtures.Fixture):
    ...
    def get_interested_computes(self, refresh=False):
        ...
        for control_node in self.inputs.bgp_ips:
            inspect_h = self.connections.cn_inspect[control_node]
            peers = inspect_h.get_cn_ri_membership(vn_fq_name=self.vn_fq_name)

        # peers can include control-only nodes, we need only compute 
        computes = list(set(peers) & \
                        set(self.inputs.compute_names))
        self._interested_computes = [self.inputs.host_data[x]['host_ip'] \
                                     for x in computes ]
