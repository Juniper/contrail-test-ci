    ...
    def verify_port_api_data(self, port_details, action='create', expected_result=None):
        ...
        for port in range(len(port_list_api['virtual-machine-interfaces'])):
            ...
            if not match_flag:
                ...
                rows_detail = self.ui.find_element('inline', 'class', browser=row_fluid, elements=True)
                for detail in range(len(rows_detail)):
                    ...
                    if key == 'Mirror to':
                        for text in range(len(value)):
                            if value[text].startswith('Routing Instance'):
                                ...
                                value_multi_string = re.search('(\w+\s+\w+\s+\w+)\s+\: (.*)', value[text])
                                value_double_string = re.search('(\w+\s+\w+)\s+\: (.*)', value[text])
                ...
                if 'fq_name' in api_data_basic:
                    complete_api_data.append(
                        {'key': 'Name', 'value': api_data_basic['fq_name'][2]})
                display_name = api_data_basic.get('display_name')
                if display_name:
                    complete_api_data.append(
                ...
                if 'ecmp_hashing_include_fields' in api_data_basic:
                    ...
                    if ecmp_fields:
                        ...
                        complete_api_data.append({'key': 'ECMP_Hashing_Fields', 'value': value.rstrip(', ')})
                ...
                if 'virtual_machine_interface_properties' in api_data_basic:
                    ...
                    if vmi_props:
                        if vmi_props['local_preference']:
                            complete_api_data.append(
                                {'key': 'Local_Preference', 'value': str(vmi_props['local_preference'])})
                ...
                if action == 'create':
                    ...
                    for result in expected_result:
                        if re.search(str(result), str(dom_arry_basic)) and \
                            re.search(str(result), str(complete_api_data)):
                            self.logger.info(
                                "%s of port matched on both WebUI and API after editing" % (result))
                        else:
                            self.logger.error(
                                "%s of port match failed on both WebUI and API after editing" % (result))
                            result = result and False
                    return result
        return result
