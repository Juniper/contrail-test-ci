...
class WebuiTest:
    ...
    def verify_global_api_data(self, expected_result=None):
        ...
        if global_config_forwarding:
           ...
           if vrouter_config:
               forward_mode = vrouter_config.get('forwarding_mode').title()
               if forward_mode:
                   if '_' in forward_mode:
                       forward_mode = forward_mode.replace('_', ' and ')
                   else:
                       forward_mode = forward_mode + ' Only'
               else:
           ...
           if flow_export_rate:
               complete_api_data.append(
                   {'key': 'Flow_Export_Rate', 'value': flow_export_rate})
        if global_config_bgp:
            ...
            if bgp_system_config:
                ...
                if grace_restart['enable']:
                    complete_api_data.append({'key': 'Graceful_Restart', 'value': 'Enabled'})
                    bgp_helper = 'Enabled' if grace_restart('bgp_helper_enable') else \
                                 'Disabled'
                    complete_api_data.append({'key': 'BGP_Helper', 'value': bgp_helper})
                    complete_api_data.append({'key': 'Restart_Time(secs)',
                                            'value': grace_restart('restart_time')})
                    complete_api_data.append({'key': 'LLGR_Time(secs)',
                                            'value': grace_restart('long_lived_restart_time')})
                    complete_api_data.append({'key': 'End_of_RIB(secs)',
                                            'value': grace_restart('end_of_rib_timeout')})
                else:
                    complete_api_data.append({'key': 'Graceful_Restart', 'value': 'Disabled'})
        self.ui.keyvalue_list(
            complete_api_data,
            Forwarding_Mode=forward_mode,
            VxLAN_Identifier_Mode=vxlan,
            Encapsulation_Priority_Order=encap_list,
            ECMP_Hashing_Fields=ecmp_value,
            Global_ASN=asn,
            iBGP_Auto_Mesh=full_mesh,
