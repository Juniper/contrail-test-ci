        ...
        if vn_list_api:
            ...
            if api_fq_vn:
                if option == 'UUID':
                    ...
                    if allow_rpf_api_out and hash_api_out and share_api and \
                       str(prov_props_seg_out) == var_list[0] and str(prov_props_phy_out) == var_list[1] \
                       and ext_api and flood_api and multi_pol_api:
                        flag = True
                    else:
                        logger.error("Options which are under advanced option is not there under virtual-network")
                        flag = False
                elif option == 'DNS':
                ...
                if vn_list_ops_element:
                    if option == 'UUID':
                        ...
                        if ops_sub_out:
                            ...
                            out = option.strip('Subnet')
                            if out == "":
                                reg_dns = re.search("dns_server_address.*" + var_list[4], ops_sub_out.group())
                                reg_dhcp = re.search("enable_dhcp.*true", ops_sub_out.group())
                                reg_gate = re.search("default_gateway.*" + var_list[5], ops_sub_out.group())
                            else:
                                if re.search('ga', out):
                                    ...
                                    reg_dns = re.search("dhcp_option_value.*" + var_list[6] + \
                                                        ".*dhcp_option_name.*6", ops_sub_out.group())
                                    reg_dhcp = re.search("enable_dhcp.*true", ops_sub_out.group())
                                    reg_gate = re.search("default_gateway.*" + var_list[5], ops_sub_out.group())
                                else:
                        ...
                        if ops_host:
                            ...
                            flag = False
                            logger.error("network_ipam_refs is not there under contrail config in OPS")
                    elif option == 'Adv Option':
                        ops_multi_pol = vn_list_ops_element.get('multi_policy_service_chains_enabled')
                        ...
                        if str(ops_multi_pol) == 'true' and ops_allow_rpf_out and ops_hash_out \
                           and str(ops_share) == 'true' and str(ops_ext) =='true' and \
                           str(ops_flood) == 'true' and ops_static_route_out and ops_prov_seg_phy:
                            flag = True
                        else:
                            flag = False
                            logger.error("Advanced option is not there under contrail config in OPS")
                    elif option == 'DNS':
                        ...
                        if ops_fip:
                            ...
                            flag = False
                            logger.error("Floating ip pools is not there under contrail config in OPS")
                    elif re.search('RT',option):
                        import pdb;pdb.set_trace()
                        if option == 'RT':
