...
class WebuiTest:
    ...
    def verify_vn_after_edit_api(self,option,value,uuid):
        ...
        if vn_list_api:
            if option == 'UUID':
                ...
                api_fq = vn_list_api['virtual-network']['network_ipam_refs']
                api_host_out = re.search("host_routes.*prefix.*1.1.1.1/24.*next_hop.*2.2.2.2",str(api_fq))
                if api_host_out:
                   flag = 1
                else:
                   flag = 0
                ...
                if allow_rpf_api_out and hash_api == True and share_api == True and prov_props_seg == 4094 and str(prov_props_phy) == 'phy1' and ext_api == True and flood_api == True and multi_pol_api == True:
                    flag = 1
                else:
                    flag = 0
            elif option == 'DNS':
                ...
                if api_dns:
                    reg_dns = re.search("dhcp_option_value.*1.1.1.1.*dhcp_option_name.*6",str(api_dns))
                    if reg_dns:
                        flag = 1
                    else:
                        flag = 0
                ...
                if api_fip:
                    reg_fip = re.search("pool1",str(api_fip))
                    #reg_domain = re.search("default-domain",str(api_fip))
                    if reg_fip:
                        flag = 1
                    else:
                        flag = 0
                ...
                if api_rt:
                    reg_rt_no = re.search("65534:4294967295",str(api_rt))
                    reg_rt_ip = re.search("10.10.10.3:4294967295",str(api_rt))
    ...
    def verify_vn_after_edit_ops(self,option,value,uuid):
        result = True
        flag = 1
        vn_list_ops = self.ui.get_vn_detail_ops("default-domain:","admin",value)
        if vn_list_ops:
            if option == 'UUID':
                ...
                ops_sub_out = re.search("10.10.10.0.*uuid",str(ops_subnet))
                if ops_sub_out:
                ...
                ops_host_out = re.search("host_routes.*prefix.*1.1.1.1/24.*next_hop.*2.2.2.2",ops_host)
                if ops_host_out:
                    flag = 1
                else:
                    flag = 0
                ...
                ops_prov_seg_phy = re.search("\"segmentation_id\"\: 4094\, \"physical_network\"\: \"phy1\"",str(ops_prov_props))
                ...
                if ops_dns:
                    reg_dns = re.search("dhcp_option_value.*1.1.1.1.*dhcp_option_name.*6",str(ops_dns))
                    flag = 1
                else:
                    flag = 0
                ...
                if ops_fip:
                    reg_fip =  re.search("pool1",str(ops_fip))
                    #reg_domain = re.search("demo",str(ops_fip))
                    if reg_fip:
                        flag = 1
                    else:
                        flag = 0
                ...
                if ops_rt:
                    reg_rt_no = re.search("65534:4294967295",str(ops_rt))
                    reg_rt_ip = re.search("10.10.10.3:4294967295",str(ops_rt))
                    if reg_rt_no or reg_rt_ip:
                        flag = 1
                    else:
                        flag = 0
    ...
    def verify_vn_after_edit_ui(self,option,value):
        ...
        try:
            ...
            if option == 'UUID':
                ...
                if str(reg_shared.group(1)).strip('-') == 'Enabled' and str(reg_ext.group(1)).strip('-') == 'Enabled' and str(reg_allow_trans.group(1)).strip('-') == 'Enabled' and str(reg_reverse.group(1)).strip('-') == 'Enabled' and str(reg_multi_chain.group(1)).strip('-') == 'Enabled' and str(reg_hash and reg_prov.group(1)).strip('-') == 'Physical Network: phy1 , VLAN: 4094' :
                ...
                regexp = "1.1.1.1"
                regexp_dns = re.search(regexp,value)
                if regexp_dns:
                ...
                regexp_rt_no = re.search('65534:4294967295',str(value))
                regexp_rt_ip = re.search('10.10.10.3:4294967295',str(value))
    ...
    def edit_vn_with_subnet(self,category):
		...
		self.ui.click_element('ui-accordion-subnets-header-0','id')
                ...
                if data_len> 3 :
                    ...
                    index = 0
                cidr[index].send_keys("10.10.10.0/24")
                pool[index].send_keys("10.10.10.5-10.10.10.10")
                if category == 'Subnet':
                    def_gate = self.browser.find_elements_by_xpath("//input[contains(@name,'default_gateway')]")
                    def_gate[index].clear()
                    def_gate[index].send_keys("10.10.10.1")
    ...
    def edit_vn_with_host_route(self,button,tc):
        result = True
        option = "Networks"
        try:
            self.edit_vn_result = self.edit_option(option,'edit')
		...
		self.ui.click_element('host_routes','id')
                self.ui.wait_till_ajax_done(self.browser)
                if button == 'add':
                    add_link = self.browser.find_elements_by_xpath("//a[contains(@class,'editable-grid-add-link')]")
                    add_link[1].click()
                    if tc == 'pos':
                        prefix = self.browser.find_element_by_xpath("//input[contains(@name,'prefix')]")
                        prefix.send_keys("1.1.1.1/24")
                        next_hop = self.browser.find_element_by_xpath("//input[contains(@name,'next_hop')]")
                        next_hop.send_keys("2.2.2.2")
                    else:
                        prefix = self.browser.find_element_by_xpath("//input[contains(@name,'prefix')]")
                        prefix.send_keys("1.1.1.1")
                        next_hop = self.browser.find_element_by_xpath("//input[contains(@name,'next_hop')]")
                        next_hop.send_keys("2.2.2.2")
    ...
    def edit_vn_with_adv_option(self,category,tc):
        result = True
        option = "Networks"
        try:
            self.ui.wait_till_ajax_done(self.browser)
            if not self.ui.click_configure_networks():
                result = result and False
            if category == 1:
                add = self.browser.find_element_by_xpath("//i[contains(@class,'icon-plus')]")
                add.click()
                time.sleep(3)
                self.browser.find_element_by_xpath("//input[contains(@name,'display_name')]").send_keys('vn1')
                self.browser.find_element_by_id('ui-accordion-subnets-header-0').click()
                time.sleep(3)
                self.browser.find_element_by_class_name("icon-plus").click()
                self.browser.find_element_by_xpath("//input[contains(@name,'user_created_cidr')]").send_keys('10.10.10.0/24')
                time.sleep(3)
                save_btn= self.browser.find_element_by_id("configure-networkbtn1")
                save_btn.click()

            self.edit_vn_result = self.edit_option(option,'edit')
            if self.edit_vn_result:
		self.ui.click_element('advanced_options','id')
                check = self.browser.find_element_by_xpath("//input[contains(@name,'is_shared')]")
                check.click()
                ext = self.browser.find_element_by_xpath("//input[contains(@name,'router_external')]")
                ext.click()
                allow = self.browser.find_element_by_xpath("//input[contains(@name,'allow_transit')]")

                allow.click()
                flood = self.browser.find_element_by_xpath("//input[contains(@name,'flood_unknown_unicast')]")
                flood.click()
                multi_sc = self.browser.find_element_by_xpath("//input[contains(@name,'multi_policy_service_chains_enabled')]")
                multi_sc.click()

                hash = self.browser.find_element_by_xpath("//div[contains(@id,'s2id_ecmp_hashing_include_fields_dropdown')]")
                hash.click()
                route_table = self.browser.find_element_by_xpath("//li[contains(@class,'select2-highlighted')]")
                route_table.click()
                if tc == 'pos-phy':
                    ...
                    route_table = self.browser.find_element_by_xpath("//li[contains(@class,'select2-highlighted')]")
                    route_table.click()
                    prov_net = self.browser.find_element_by_xpath("//input[contains(@name,'user_created_sriov_enabled')]")
                    prov_net.click()
                    phy_net = self.browser.find_element_by_xpath("//input[contains(@name,'physical_network')]")
                    phy_net.send_keys('phy1')
                    seg_id = self.browser.find_element_by_xpath("//input[contains(@name,'segmentation_id')]")
                    seg_id.send_keys('4094')
                    ...
                    seg_id = self.browser.find_element_by_xpath("//input[contains(@name,'segmentation_id')]")
                    seg_id.clear()
                    seg_id.send_keys('4094abcd49494')

		self.ui.click_element('configure-networkbtn1','id')
                if tc == 'neg-phy':
                    warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,'advanced')]")
                    if warn_button.get_attribute('style') == "":
                        self.ui.click_on_cancel_if_failure('cancelBtn')
                        self.wait_till_ajax_done(self.browser)
                        return result
    ...
    def edit_vn_with_dns(self,button,tc):
        result = True
        option = "Networks"
        try:
            self.edit_vn_result = self.edit_option(option,'edit')
		...
		self.ui.click_element('ui-accordion-dns_servers-header-0','id')
		    ...
		    self.ui.click_element('user_created_dns_servers','id')
                    text = self.browser.find_element_by_xpath("//input[contains(@name,'ip_address')]")
                    if tc == 'pos':
                        text.send_keys("1.1.1.1")
                    else:
                        text.send_keys("1,1,1,1")
                else:
                    minus = self.browser.find_elements_by_xpath("//i[contains(@class,'icon-minus')]")
		...
		self.ui.click_element('configure-networkbtn1','id')
                self.ui.wait_till_ajax_done(self.browser)
                if tc == 'neg':
                    warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,'dnsServers')]")
                    if warn_button.get_attribute('style') == "":
                        self.ui.click_on_cancel_if_failure('cancelBtn')
                        self.wait_till_ajax_done(self.browser)
                        return result
    ...
    def edit_vn_with_fpool(self,button):
        result = True
        option = "Networks"
        try:
            self.edit_vn_result = self.edit_option(option,'edit')
		...
		self.ui.click_element('fip_pool_accordian','id')
                ...
                if button == 'add':
                    ...
                    add_link[3].click()
                    self.ui.wait_till_ajax_done(self.browser)
                    pool = self.browser.find_element_by_xpath("//input[contains(@placeholder,'Enter Pool Name')]")
                    pool.send_keys('pool1')
                    self.ui.wait_till_ajax_done(self.browser)
		    self.ui.click_element('s2id_projects_dropdown','id')
                    select = self.browser.find_element_by_xpath("//li[contains(@class,'select2-highlighted')]")
                    self.project = select.text
                    select.click()
                    choice = self.browser.find_element_by_xpath("//li[contains(@class,'select2-search-choice')]")
                else:
                    minus = self.browser.find_elements_by_xpath("//i[contains(@class,'icon-minus')]")
                    index = len(minus)
                    minus[index-1].click()
    ...
    def edit_vn_with_route_target(self,button,asn_opt,tc,rt_type):
        result = True
        option = "Networks"
        try:
            self.edit_vn_result = self.edit_option(option,'edit')
			...
			route = self.ui.find_element('ui-accordion-import_route_targer_accordian-header-0','id')
                        route.click()

                        ind = 6
                        route.click()
                        time.sleep(5)
                        rt = self.browser.find_elements_by_xpath("//div[contains(@id,'import_route_target_vcfg')]")
                        user_irt = self.browser.find_elements_by_xpath("//div[contains(@id,'user_created_import_route_targets')]")
                    self.ui.wait_till_ajax_done(self.browser)
                    if button == 'add':
                        add_link = self.browser.find_elements_by_xpath("//a[contains(@class,'editable-grid-add-link')]")
                        add_link[ind].click()
                        self.ui.wait_till_ajax_done(self.browser)
                        asn = self.browser.find_element_by_xpath("//input[contains(@name,'asn')]")
                            if tc == 'pos':
                                if asn_opt == 'number':
                                    asn.send_keys('65534')
                                else:
                                    asn.send_keys('10.10.10.3')
                                target = self.browser.find_element_by_xpath("//input[contains(@name,'target')]")
                                target.send_keys('4294967295')
                            else:
                                if asn_opt == 'number':
                                    asn.send_keys('65534ab')
                                else:
                                    asn.send_keys('10,10,10,3')
                                target = self.browser.find_element_by_xpath("//input[contains(@name,'target')]")
                                target.send_keys('4294967295abc')
                    else:
                        if rt_type == 'IRT':
                            self.ui.click_element('import_route_targer_accordian','id')
                            time.sleep(5)
                            irt = self.browser.find_elements_by_xpath("//div[contains(@id,'import_route_target_vcfg')]")
                            user_irt = self.browser.find_elements_by_xpath("//div[contains(@id,'user_created_import_route_targets')]")
                            #time.sleep(10)
                        minus = self.browser.find_elements_by_xpath("//i[contains(@class,'icon-minus')]")
                        index = len(minus) - 1
		    ...
		    self.ui.click_element('configure-networkbtn1','id')
                    ...
                    if tc == 'neg':
                        if rt_type == 'RT':
                            warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,'route_target_vcfg')]")
                        elif rt_type == 'ERT':
                            warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,'export_route_target_vcfg')]")
                        elif rt_type == 'IRT':
                            warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,'import_route_target_vcfg')]")
                            #warn_button = self.browser.find_element_by_xpath("//span[contains(@data-bind,warn)]")
                        if warn_button.get_attribute('style') == "":
                            self.ui.click_on_cancel_if_failure('cancelBtn')
                            self.ui.wait_till_ajax_done(self.browser)
                            return result
