...
class WebuiTest:
    ...
    def delete_svc_appliance_set(self):
        ...
        for row in rows:
            element_text = self.ui.find_element('div', 'tag', elements=True,
                                       browser=row)[2].text
        ...
        try:
            for appl in appliance_list:
                ...
                if not self.ui.click_on_create(
                        ...
                        select_project=False):
                    result = result and False
                self.ui.send_keys(appl, 'display_name', 'name', clear=True)
                self.ui.send_keys(appl_lbdriver, 'custom-combobox-input',
                                 'class', clear=True)
                self.ui.send_keys(appl_mode, 'service_appliance_ha_mode',
                                 'name', clear=True)
                self.ui.click_element('ui-accordion-header-icon', 'class')
                self.ui.click_element('editable-grid-add-link', 'class')
                self.ui.send_keys(appl_key, 'key', 'name', clear=True)
                self.ui.send_keys(appl_value, 'value', 'name', clear=True)
        ...
        try:
            for appl in appliance_list:
                ...
                if not self.ui.click_on_create(
                    ...
                    result = result and False
                self.ui.send_keys(appl, 'display_name', 'name', clear=True)
                self.ui.send_keys(appl_ip, 'service_appliance_ip_address',
                                 'name', clear=True)
                self.ui.click_element('ui-id-5')
                self.ui.send_keys(appl_uname, 'username', 'name', clear=True)
                self.ui.send_keys(appl_pword, 'password', 'name', clear=True)
                self.ui.click_element('ui-id-6')
                self.ui.click_element('editable-grid-add-link', 'class')
                self.ui.send_keys(appl_key, 'key', 'name', clear=True)
                self.ui.send_keys(appl_value, 'value', 'name', clear=True)
