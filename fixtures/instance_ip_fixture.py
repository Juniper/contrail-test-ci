...
class InstanceIpFixture (ContrailFixture):
   ...
   def _read_vnc_obj (self):
       obj = self._vnc.get_instance_ip(self.uuid)
   ...
   def _create (self):
       ...
       self.uuid = self._ctrl.create_instance_ip(
           **self._args)

   def _delete (self):
       self.logger.info('Deleting %s' % self)
       self._ctrl.delete_instance_ip(
           obj=self._obj, uuid=self.uuid)

   def _update (self):
       self.logger.info('Updating %s' % self)
       self._ctrl.update_instance_ip(
   ...
   def _verify_not_in_api_server (self):
       if self._vnc.get_instance_ip(self.uuid):
