...
from barbican_test import CreateContainer
...
class LBaasV2Fixture(LBBaseFixture):
    ...
    def create(self):
        ...
        if self.listener_uuid:
            ...
            container_ref = None
            if self.default_tls_container and not self.tls_container:
                cert_key_payload = CreateCertsKeys.createCertificate()
                self.create_container(self.default_tls_container, cert_payload=cert_key_payload[0], pkey_payload=cert_key_payload[1])
                self.create_container_cert()
                self.add_neutron_user_to_acl()
                container_ref = self.container_certs.container_ref
    ...
    def create_container(self, container_name='tls_container', cert_name='certificate',
        cert_payload='certificate', pkey_name='private_key', pkey_payload='priavte_key'):
        self.tls_container = CreateContainer(connections=self.connections, container_name=container_name,
            cert_name=cert_name, cert_payload=cert_payload,
            pkey_name=pkey_name, pkey_payload=pkey_payload)
        self.tls_container.setUp()
    ...
    def create_container_cert(self):
        self.container_certs = self.tls_container.create_container_certificate()
