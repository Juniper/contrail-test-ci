...
class ContrailVncApi(object):
    ...
    def create_floating_ip(self, vn_obj, project_obj, **kwargs):
        #Create FIP Pool
        fip_pool_name = 'fip_pool'   
        fip_pool = FloatingIpPool(fip_pool_name, vn_obj)
        self._vnc.floating_ip_pool_create(fip_pool)
        owner = kwargs.get('owner')

        fip_obj = FloatingIp(get_random_name('fip'), fip_pool)
    ...
    def create_floatingip(self, fip_pool_vn_id, 
        ...
        proj_obj = self.vnc_project 
        fip_obj = self.create_floating_ip(pool_obj, proj_obj)
        if port_id:
            fip_id = fip_obj.uuid
