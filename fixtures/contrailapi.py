...
class ContrailVncApi:
    ...
    def create_vmobj_in_api_server(self,vm_uuid=None):
        vm = VirtualMachine()
        if vm_uuid:
            vm.set_uuid(vm_uuid)
        self._vnc.virtual_machine_create(vm)
    #end create_vmobj_in_api_server
    
    def delete_vmobj_in_api_server(self,vm_uuid):
        self._vnc.virtual_machine_delete(id=vm_uuid)
    #end delete_vmobj_in_api_server
