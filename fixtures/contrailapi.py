...
class ContrailVncApi:
    ...
    def _create_network(self, vn_name, **kwargs):
        ...
        if 'vn_subnets' in kwargs and kwargs['vn_subnets']:
            if 'ipam_fq_name' in kwargs['vn_subnets'] and not kwargs['vn_subnets']['ipam_fq_name']:
                ipam_fq_name = kwargs['vn_subnets']['ipam_fq_name']
            else:
                ipam_fq_name = NetworkIpam().get_fq_name()
            for subnet in kwargs['vn_subnets']:
                self.create_subnet(subnet, vn_obj, ipam_fq_name)
    ...
    def list_networks(self, project_id=None):
        if project_id:
            return self._vnc.virtual_networks_list(parent_id=project_id)
        else:
    ...
    def create_subnet(self, subnet, net_id, ipam_fq_name=None,
        enable_dhcp=True, disable_gateway=False, dhcp_option_list=False):
        if isinstance(net_id, object):
            vn_obj = net_id
        else:
            vn_obj = self._vnc.virtual_network_read(id=net_id)
        ...
        network, prefix = subnet['cidr'].split('/')
        ipam_sn = IpamSubnetType(
            subnet=SubnetType(network, int(prefix)))
        if dhcp_option_list:
           ipam_sn.set_dhcp_option_list(dhcp_option_list)
        if not enable_dhcp:
           ipam_sn.set_enable_dhcp(enable_dhcp)
    ...
    def list_subnets(self, project_id=None):
        if project_id:
            return self._vnc.subnets_list(parent_id=project_id)
        else:
