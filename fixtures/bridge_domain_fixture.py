...
class BDFixture(fixtures.Fixture):
    '''
    Bridge Domain Fixture
    '''

    def __init__(self, connections, parent_obj, bd_name=None, bd_uuid=None, **kwargs):
        ...
        self.bd_name = bd_name or get_random_name('bd_' + self.project_name)
    ...
    def verify_bd_in_api_server(self):
        ...
        for cfgm_ip in self.inputs.cfgm_ips:
            ...
            if not self.api_s_bd_obj:
                self.logger.debug("Bridge Domain %s is not found in API-Server" %
                                 (self.bd_name))
    ...
    def verify_bd_for_vmi_in_computes(self, vmi_uuid):
        ...
        if vmi_uuid:
            ...
            vmi_in_vrouter = self.agent_inspect[
                vmi_host_ip].get_vrouter_interface_by_name(vmi_in_agent['name'])
